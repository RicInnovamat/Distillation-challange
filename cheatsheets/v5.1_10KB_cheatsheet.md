...
STEP 6. FORCED SUBSTITUTION CHECK
Set all variables equal: x=y=z=w=u=a. Evaluate both sides of Eq1 and Eq2.
- If Eq1 becomes a=a (holds) and Eq2 also becomes a=a → this does NOT prove TRUE, continue to default.
- If Eq1 holds but Eq2 does NOT hold under this substitution → confirms FALSE.
- Now try left-projection (p*q=p) and right-projection (p*q=q) on BOTH equations:
If Eq1 holds under one of these and Eq2 also holds → answer TRUE. RULE: projection check.
