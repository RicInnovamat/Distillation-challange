...
STEP 4.5. STRUCTURAL TRUE TIEBREAKER (NEW in v.Opus-2.1)
Apply ONLY if no route in STEP 2 fired, no route in STEP 3 fired, and no witness in STEP 4 fired. This is the "all-quiet" regime: the witnesses themselves have already failed to refute Eq1 → Eq2.

Use the feature vector from 2.8. Let VEq2 = set of variables appearing in Eq2.

If Eq1 is bare AND any of the following holds on the product side P of Eq1:
- maxMult(P) >= 3 (some variable occurs 3 or more times on P), OR
- some variable in P is NOT in VEq2 (one-sided ghost variable), OR
- xCount >= 2 AND rhsVars >= 3 (x recurs with multiple other variables),
then Eq1 imposes a strongly constraining identity not easily falsifiable by small witnesses. Answer TRUE.
RULE: structural tiebreaker
